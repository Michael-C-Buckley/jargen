...
@cli.option(
    "-c",
    "--configfile",
    help="Specify path (including file name) to the config YAML file. By default, routegen assumes a config.yml file in the current working directory",
    type=str,
    default="config.yml",
)
def main(configfile: str) -> None:
    ...
    for platform in config["basic"]["platforms"]:
        platforms[platform](
            path=path.join(path.dirname(__file__), "output"), routes=routes
        )
