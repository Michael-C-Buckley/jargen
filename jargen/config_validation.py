...
def basic_validation_caller(
    config: dict | list, schema: dict | list, types: dict | list
):
    ...
    def basic_validation_list():
        ...
        try:
            ...
            if len([dict(t) for t in {tuple(d.items()) for d in config}]) != len(
                config
            ):
                return False
    ...
    for item in config:
        ...
        if isinstance(dataType, (dict, list)):
            if not basic_validation_caller(
                config=dataContent, schema=dataSchema, types=dataType
            ):
                return False
        else:
...
def validate_base(key: str) -> Callable:
    def wrapper(f: Callable):
        def inner(
            ...
            if not basic_validation_caller(
                config=config[key], schema=schema[key], types=types[key]
            ):
                return False
...
def validateConfig(
    ...
    enabledTests = {"Test to validate basic configuration": validate_basic}
